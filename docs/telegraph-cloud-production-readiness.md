...
| --- | --- | --- |
| Git candidate / Pages Preview | GitHub’s Cloudflare Pages check reports commit `a21a692` deployed successfully to the immutable Preview URL `https://53ff076f.telestorage.pages.dev` (and its branch Preview URL). Cloudflare Pages account settings themselves were not available to this environment. | The reviewed code is deployed to Preview; this does not attest to its configuration or authorize mutation testing. |
